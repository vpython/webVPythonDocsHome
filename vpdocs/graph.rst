...
``aria_label`` (string)
   A text label announced by screen readers for the graph. Default is ``'WebVPython Graph'``::

      g1 = graph(title='Pendulum Angle', aria_label='Pendulum angle over time')

``aria_focusable`` (boolean)
   If ``True``, the graph element can receive keyboard focus (via Tab key), making it reachable by screen reader users. Default is ``False``::

      g1 = graph(title='Pendulum Angle', aria_label='Pendulum angle over time', aria_focusable=True)
...
..  py:function:: set_crosshairs_disabled(True)

Disables the hover crosshairs on all graphs. By default, moving the mouse over a graph displays crosshair lines and the coordinates of the nearest data point. Call this once at the start of a program to turn that off::

   set_crosshairs_disabled(True)

Call with ``False`` to re-enable crosshairs. Only applies to fast-mode graphs (``fast=True``, the default).
