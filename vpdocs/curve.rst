...
.. py:function:: curve( pos=[ vec(-1,3,2), vec(2,0,-1) ] ,  color=color.yellow)
   ...
   :type radius: scalar
   :param size: Changing size scales the entire curve. Initially size is <1,1,1>, regardless of the actual dimensions of the curve.
   :type size: vector
...
It is also possible to append a list of vectors:

.. code-block:: python

    mycurve.append(pos=[vec(1,1,1), vec(-1,-1,-1)])
