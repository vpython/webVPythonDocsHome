...
``aria_label`` (string)
   The accessible label announced by screen readers for the canvas element. Default is ``'WebVPython canvas'``. Override this when a program has multiple canvases or when a more descriptive label is needed::

      scene = canvas(aria_label="Bouncing ball simulation")

``caption_aria_live`` (string)
   Controls how screen readers announce changes to the caption area (below the canvas). Default is ``'assertive'``, which interrupts the screen reader immediately. Set to ``'polite'`` if the caption updates frequently and interruptions would be disruptive::

      scene = canvas(caption_aria_live='polite')

``title_aria_live`` (string)
   Controls how screen readers announce changes to the title area (above the canvas). Default is ``'assertive'``. Same values and trade-offs as ``caption_aria_live``.
